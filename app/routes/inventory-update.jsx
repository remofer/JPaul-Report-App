...
const fetchProducts = async (shop, accessToken) => {
  let nextLink = `https://${shop}/admin/api/2023-01/products.json?limit=250`, products = [];
  while (nextLink) {
    const res = await fetch(nextLink, { headers: { "X-Shopify-Access-Token": accessToken } });
    if (!res.ok) throw new Error(`Failed to fetch products: ${res.statusText}`);
    const data = await res.json();
    products.push(...data.products);
    nextLink = res.headers.get("link")?.includes('rel="next"')
      ? res.headers.get("link").split(",").find((l) => l.includes('rel="next"')).split(";")[0].trim().slice(1, -1)
      : null;
  }
  return products.reduce((acc, p) => {
    acc[p.handle] = p.variants.reduce((map, v) => ((map[v.sku] = v.inventory_item_id), map), {});
    return acc;
  }, {});
};
...
export const action = async ({ request }) => {
  ...
  try {
    ...
    const shop = "checkoutlatern.myshopify.com";
      ...
      (acc, product) => {
        ...
        !inventoryId
          ...
          : acc.updates.push(
              ...
              }).then((res) => (!res.ok ? res.json().then((body) => acc.errors.push(body.errors)) : null))
            );
        return acc;
      },
      { updates: [], errors: [] }
